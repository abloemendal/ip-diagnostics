...
import { fetchIpData, fetchPublicIpOnly, fetchBothIps, normalizeIpOnly } from './ip-services.js';

const els = {
  heroKicker: document.getElementById('hero-kicker'),
  ip: document.getElementById('ip-address'),
  ipContainer: document.getElementById('ip-container'),
  ipV4Block: document.getElementById('ip-v4-block'),
  ipV6Block: document.getElementById('ip-v6-block'),
  ipV4: document.getElementById('ip-address-v4'),
  ipV6: document.getElementById('ip-address-v6'),
  copyV4Btn: document.getElementById('copy-ipv4'),
  copyV6Btn: document.getElementById('copy-ipv6'),
...
const state = {
  ...
  speedTest: null,
  ipCopyResetTimer: 0,
  ipV4CopyResetTimer: 0,
  ipV6CopyResetTimer: 0,
...
async function init() {
  ...
  bindIpV4CopyToClipboard();
  bindIpV6CopyToClipboard();
  ...
  let seededIp = null;
  let ipsData = null;
  let detectedPrimaryIp = null;  // Will be set based on what fetchIpData() returns
  const ipSeedTask = fetchBothIps()
    .then((ips) => {
      // Store debug info for RAW JSON output
      ipsFetchDebug = {
        ipv4_result: ips.ipv4,
        ipv6_result: ips.ipv6,
        fetch_errors: ips.errors
      };

      // Store both IPs (don't assume which is primary yet)
      const primaryIp = ips.ipv4 || ips.ipv6;
      ...
      ipsData = ips;
      seededIp = primaryIp;
      // Will be updated later based on what fetchIpData() returns
      state.currentIp = String(primaryIp);

      // If both IPs are the same, clear the secondary one (prevent duplicates)
      if (ips.ipv4 && ips.ipv6 && String(ips.ipv4) === String(ips.ipv6)) {
        state.ipv6 = null;
        ipsFetchDebug.duplicate_detected = true;
      }

      // Record if we only got one IP type
      if ((ips.ipv4 && !ips.ipv6) || (!ips.ipv4 && ips.ipv6)) {
        ipsFetchDebug.single_ip_only = true;
      }

      // Display both IPs in separate blocks (only if they're different)
      if (state.ipv4 && state.ipv6) {
        // Show dual-stack display
        els.ipContainer.classList.remove('hidden');
        els.ip.classList.add('hidden');
        els.ipV4Block.classList.remove('hidden');
        els.ipV6Block.classList.remove('hidden');
        setAsciiText(els.ipV4, state.ipv4, { onFrame: fitIpHeading, onDone: fitIpHeading });
        setAsciiText(els.ipV6, state.ipv6, { onFrame: fitIpHeading, onDone: fitIpHeading });
      } else {
        // Show single IP in fallback element
        els.ip.classList.remove('hidden');
        els.ipContainer.classList.add('hidden');
        setAsciiText(els.ip, primaryIp, { onFrame: fitIpHeading, onDone: fitIpHeading });
      }
      return primaryIp;
    })
    .catch(() => null);
  ...
  try {
    // Wait for IP addresses to be fetched first
    await ipSeedTask;

    // Fetch geolocation for the IP system is currently using
    const ipData = await fetchIpData();
    state.ipApi = ipData;

    // Determine which IP was returned (this is the "actual" primary)
    const returnedIp = String(ipData?.ip || '');
    detectedPrimaryIp = returnedIp;

    // If we have both IPv4 and IPv6, fetch detailed data for BOTH
    if (state.ipv4 && state.ipv6) {
      // Determine which IP was returned as primary
      const ipv4Match = returnedIp === String(state.ipv4);
      const ipv6Match = returnedIp === String(state.ipv6);

      // Store the data for whichever IP was returned
      if (ipv4Match) {
        state.ipApiV4 = ipData;
        detectedPrimaryIp = state.ipv4;
      } else if (ipv6Match) {
        state.ipApiV6 = ipData;
        detectedPrimaryIp = state.ipv6;
      }

      // Now fetch data for the OTHER IP
      const otherIp = ipv4Match ? state.ipv6 : state.ipv4;
      try {
        const otherData = await fetchIpData(otherIp);

        // VALIDATE: Check if returned data matches the IP we requested
        const returnedOtherIp = otherData?.ip;
        const otherIpMatches = returnedOtherIp && String(returnedOtherIp) === String(otherIp);

        if (!otherIpMatches && returnedOtherIp) {
          ipsFetchDebug.geolocation_mismatch = {
            requested: otherIp,
            returned: returnedOtherIp,
            fallback_to_normalized: true
          };
        }

        // Use the data if it matches, otherwise use normalized data
        const dataToStore = otherIpMatches ? otherData : normalizeIpOnly(otherIp);

        if (ipv4Match) {
          state.ipApiV6 = dataToStore;
        } else {
          state.ipApiV4 = dataToStore;
        }
      } catch {
        // If secondary lookup fails, use minimal data for it
        if (ipv4Match) {
          state.ipApiV6 = normalizeIpOnly(state.ipv6);
        } else {
          state.ipApiV4 = normalizeIpOnly(state.ipv4);
        }
      }
    } else {
      state.ipApi = ipData;
...
function renderIpData(ipData, client) {
  ...
  // Only update header if we DON'T have both IPs (dual-stack was already set in init)
  if (!(state.ipv4 && state.ipv6)) {
  ...
  // Determine which IP data objects to use
  const ipV4Data = state.ipApiV4;
  const ipV6Data = state.ipApiV6;
  const hasDualStack = ipV4Data && ipV6Data;

  // Build network rows - include both IPv4 and IPv6 with full details if available
  ...
  if (hasDualStack) {
    ...
    networkEntries.push(
      ...
      ['[IPv4] TYPE', ipV4Data.type],
      ['[IPv4] ISP', ipV4Data.connection?.isp],
      ['[IPv4] ORG', ipV4Data.connection?.org],
      ['[IPv4] ASN', ipV4Data.connection?.asn],
      ['[IPv4] DOMAIN', ipV4Data.connection?.domain],
      ['[IPv4] HOSTNAME', ipV4Data.connection?.hostname],
      ['[IPv4] ROUTEABLE', ipV4Data.type === 'IPv4' ? 'YES' : 'UNKNOWN'],
      ['[IPv4] RISK FLAGS', detectNetworkRiskFlags(ipV4Data)]
    );

    // Show IPv6 details
    networkEntries.push(
      ['[IPv6] ADDRESS', ipV6Data.ip],
      ['[IPv6] TYPE', ipV6Data.type],
      ['[IPv6] ISP', ipV6Data.connection?.isp],
      ['[IPv6] ORG', ipV6Data.connection?.org],
      ['[IPv6] ASN', ipV6Data.connection?.asn],
      ['[IPv6] DOMAIN', ipV6Data.connection?.domain],
      ['[IPv6] HOSTNAME', ipV6Data.connection?.hostname],
      ['[IPv6] ROUTEABLE', ipV6Data.type === 'IPv6' ? 'YES' : 'UNKNOWN'],
      ['[IPv6] RISK FLAGS', detectNetworkRiskFlags(ipV6Data)]
    );
  } else {
    // Single IP - show generic labels
    networkEntries.push(
      ['IP', ipData.ip],
      ['TYPE', ipData.type],
  ...
  renderRows(els.networkRows, networkEntries, els.networkCount);

  // Show geo data for primary IP
...
function bindIpV4CopyToClipboard() {
  if (!els.copyV4Btn) return;

  const handleCopy = async () => {
    const ipText = String(state.ipv4 || '').trim();
    if (!ipText) return;

    // Show button feedback
    if (state.ipV4CopyResetTimer) window.clearTimeout(state.ipV4CopyResetTimer);
    els.copyV4Btn.textContent = 'COPIED';
    els.copyV4Btn.classList.add('copied');

    let copied = legacyCopyText(ipText);
    if (!copied && navigator.clipboard?.writeText) {
      try {
    ...
    state.ipV4CopyResetTimer = window.setTimeout(() => {
      els.copyV4Btn.textContent = 'COPY';
      els.copyV4Btn.classList.remove('copied');
      state.ipV4CopyResetTimer = 0;
    }, copied ? 1500 : 1000);
  };

  els.copyV4Btn.addEventListener('pointerdown', (event) => {
    if (event.button !== 0) return;
    handleCopy().catch(() => {});
  });
  els.copyV4Btn.addEventListener('keydown', (event) => {
    if (event.key !== 'Enter' && event.key !== ' ') return;
    event.preventDefault();
    handleCopy().catch(() => {});
  });
...
function bindIpV6CopyToClipboard() {
  if (!els.copyV6Btn) return;

  const handleCopy = async () => {
    const ipText = String(state.ipv6 || '').trim();
    if (!ipText) return;

    // Show button feedback
    if (state.ipV6CopyResetTimer) window.clearTimeout(state.ipV6CopyResetTimer);
    els.copyV6Btn.textContent = 'COPIED';
    els.copyV6Btn.classList.add('copied');
    ...
    state.ipV6CopyResetTimer = window.setTimeout(() => {
      els.copyV6Btn.textContent = 'COPY';
      els.copyV6Btn.classList.remove('copied');
      state.ipV6CopyResetTimer = 0;
    }, copied ? 1500 : 1000);
  };

  els.copyV6Btn.addEventListener('pointerdown', (event) => {
    if (event.button !== 0) return;
    handleCopy().catch(() => {});
  });
  els.copyV6Btn.addEventListener('keydown', (event) => {
    if (event.key !== 'Enter' && event.key !== ' ') return;
    event.preventDefault();
    handleCopy().catch(() => {});
  });
...
function renderRawJson(extra = {}) {
  ...
  // Include debug info about IP fetching
  if (ipsFetchDebug) {
    rawData.ipFetchDebug = ipsFetchDebug;
  }
  ...
  // Include IP data (handle both single IP and dual IP scenarios)
  if (state.ipApiV4 || state.ipApiV6) {
    rawData.ipData = {};
    if (state.ipApiV4) rawData.ipData.ipv4 = state.ipApiV4;
    if (state.ipApiV6) rawData.ipData.ipv6 = state.ipApiV6;
  } else if (state.ipApi) {
    rawData.ipApi = state.ipApi;
  }
